...
# 定义物理量
    def zero_padding(self, raw_img):
        ...
        rand_pha = np.random.rand(h,w)
        rand_pha = np.exp(rand_pha * 1j)
        pad_img = np.multiply(pad_img, rand_pha)# 随机位相
        return pad_img
#
...
# 生成全息图复振幅Uf
    def wave(self, pad_img):
        ...
        # xr = -L / 6
        xr = 0
        phi = np.pi * ((np.power(X1 + xr, 2) + np.power(Y1 + xr, 2)) / (
                    self.lambda_ * self.z0) + 2 * self.z0 / self.lambda_)
        sphericalWave = np.exp(1j * phi) / self.z0
